...
import { compareRuns, createComparisonReport, renderComparisonHtml, renderComparisonSvg, serializeComparisonReport, summarizeExperiment } from '../lib/core/index.js'
...
test('exports redacted JSON and escaped standalone HTML', () => {
  ...
  const report = createComparisonReport(comparison, evidence, '2026-08-26T00:00:00.000Z', summarizeExperiment([comparison]))
  const json = serializeComparisonReport(report)
...
test('exports startup failures as an invalid comparison', () => {
  ...
  assert.match(renderComparisonHtml(report), /Agent failures/)
  assert.match(renderComparisonHtml(report), /has no provider\/model/)
  assert.match(renderComparisonSvg(report), /AGENT STARTUP FAILED/)
})
...
test('web, HTML, SVG and JSON preserve plugin identity separately from the preset', () => {
  ...
  const report = createComparisonReport(comparison, evidence, undefined, summarizeExperiment([comparison]))
  for (const output of [renderComparisonHtml(report), renderComparisonSvg(report), renderToStaticMarkup(React.createElement(ComparisonView, { comparison, evidence, experiment: report.experiment }))]) {
  ...
  assert.equal(JSON.parse(serializeComparisonReport(report)).comparison.candidate.presetId, 'expert-mode')
})
...
test('plugin metadata is escaped and old reports do not fabricate ownership', () => {
  ...
  const report = createComparisonReport(comparison)
  for (const output of [renderComparisonHtml(report), renderComparisonSvg(report)]) {
    assert.doesNotMatch(output, /<img src|<script>/)
    assert.match(output, /&lt;img/)
    assert.match(output, /added after execution/)
  }
  const legacy = renderComparisonHtml(createComparisonReport(compareRuns(run('a', 'Standard'), run('b', '专家模式'))))
