...
The workflow selects npm dist-tags from the package version: versions containing a prerelease suffix publish to `next`; stable versions publish to `latest`. npm may create `latest` automatically for the first package version even when another tag was requested. If that first version is a prerelease, a maintainer with account 2FA enabled must remove the accidental tag interactively with `npm dist-tag rm <package> latest`; a successful web-based `npm login` alone is insufficient when account 2FA is disabled, and the CI publishing token cannot perform this protected cleanup. Never publish an alpha, beta, or release candidate as `latest`.
