...
The workflow selects npm dist-tags from the package version: versions containing a prerelease suffix publish to `next`; stable versions publish to `latest`. Never publish an alpha, beta, or release candidate as `latest`.
