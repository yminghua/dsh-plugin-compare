...
import type { ComparisonEvidence, FileDiffEvidence, RunComparison } from './types.ts'
...
export function createComparisonReport(comparison: RunComparison, evidence?: ComparisonEvidence, generatedAt = new Date().toISOString(), experiment?: ExperimentSummary): ComparisonReport {
...
export function sanitizeComparisonReport(report: ComparisonReport): ComparisonReport {
  ...
  const safe = JSON.parse(redacted.text) as ComparisonReport
...
export function serializeComparisonReport(report: ComparisonReport): string {
  return JSON.stringify(sanitizeComparisonReport(report), null, 2)
}

export function renderComparisonHtml(report: ComparisonReport): string {
  const safe = sanitizeComparisonReport(report)
...
<body><main class="wrap"><header class="masthead"><b>DSH / PLUGIN COMPARE</b><span>Plugin comparison report</span></header><section class="card"><header class="head"><div class="eyebrow">MEASURED EVIDENCE / NOT A LEADERBOARD</div><h1>${escapeHtml(runIdentity(baseline).name)} <span>vs</span> ${escapeHtml(runIdentity(candidate).name)}</h1>${identities}<div class="verdict"><strong>${escapeHtml(reportHeadline(safe.comparison))}</strong><span>${escapeHtml(sample)}</span></div>${metrics}<p class="subtle">Token totals include input, output, cache reads and cache writes. Token change is not a cost saving.</p><p class="note"><strong>Privacy check required before sharing.</strong> Automatic redaction is best-effort, not a privacy clearance. Review prompts, paths, code, command output, account data, private URLs, and Session ids.</p></header><section class="evidence"><h2>Metric details <small>A → B${safe.experiment && safe.experiment.trials > 1 ? ' · first pair' : ''}</small></h2><div class="table-scroll"><table><thead><tr><th>Metric</th><th>A · Baseline</th><th>B · Candidate</th></tr></thead><tbody>${body}</tbody></table></div></section>${failures}${experiment}${checks}${evidence}<footer><p>${escapeHtml(safe.disclaimer)}</p><span>Generated ${escapeHtml(safe.generatedAt)} · Schema v${safe.schemaVersion} · ${safe.exportManifest.redaction.matches} secret-like value(s) redacted · zero matches is not a privacy clearance</span></footer></section></main></body></html>`
}

function renderIdentity(role: string, run: RunComparison['baseline']): string {
...
function renderCheck(role: string, check: RunComparison['baseline']['check']): string {
...
export function renderComparisonSvg(report: ComparisonReport): string {
  const safe = sanitizeComparisonReport(report)
  const { baseline, candidate, deltas } = safe.comparison
...
function renderFailure(label: string, failure: RunComparison['baseline']['failure']): string {
