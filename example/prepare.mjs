...
git(['-c', 'user.name=DSH Plugin Compare Example', '-c', 'user.email=example@invalid.local', 'commit', '--no-gpg-sign', '-m', 'fixture: intentionally broken checkout'])
...
let compareCommit = 'not available'
try { compareCommit = execFileSync('git', ['-C', join(exampleRoot, '..'), 'rev-parse', 'HEAD'], { encoding: 'utf8', stdio: 'pipe' }).trim() } catch {}
const notes = (await readFile(join(exampleRoot, 'run-notes.template.md'), 'utf8'))
  ...
  .replaceAll('{{COMPARE_COMMIT}}', () => compareCommit)
